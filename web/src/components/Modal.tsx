...
const Modal: React.FC = () => {
  ...
  return (
    <Dialog open={modal.open} onOpenChange={() => setModal((prev : any) => ({ ...prev, open: false }))} modal>
      <DialogContent
        container={container}
        style={{
          width: typeof modal.size === 'number' ? modal.size : undefined,
        }}
        className={cn(!modal.size && 'max-w-sm', typeof modal.size === 'string' && SIZES[modal.size as keyof Object])}
      >
        <DialogHeader>
